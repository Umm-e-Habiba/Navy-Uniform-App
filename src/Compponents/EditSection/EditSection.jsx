...
const EditSection = () => {
    ...
    const ToggleLeftBicep = (item) => {
        console.log("Clicked:", item);
        setSelectedOptions((prevItems) => {
            const existingItemIndex = prevItems?.leftBiceps?.findIndex(
                (i) => i?.badgeKey === item?.badgeKey
            );
    
            let leftBicepsArray = makeDeepCopy(prevItems?.leftBiceps);
            console.log("Before Toggle:", prevItems.leftBiceps);
            if (existingItemIndex !== -1) {
                leftBicepsArray = leftBicepsArray?.filter((i) => i.badgeKey !== item.badgeKey);
            } else {
                if (leftBicepsArray?.length >= 2) {
                    toast.warn("Select 2 Insignias Only");
                    return prevItems;
                } else {
                    leftBicepsArray.push(item);
                }
            }
    
            leftBicepsArray?.sort((a, b) => a.id - b.id);
    
            return {
                ...prevItems,
                leftBiceps: leftBicepsArray,
            };
        });
    };
    
    const ToggleScraf = (item) => {
        console.log("Clicked:", item);
        setSelectedOptions((prevItems) => {
            const existingItemIndex = prevItems?.scrafs?.findIndex(
                (i) => i?.badgeKey === item?.badgeKey
            );
    
            let scrafArray = makeDeepCopy(prevItems?.scrafs);
            console.log("Before Toggle:", prevItems.scrafs);
            if (existingItemIndex !== -1) {
                scrafArray = scrafArray?.filter((i) => i.badgeKey !== item.badgeKey);
            } else {
                if (scrafArray?.length >= 2) {
                    toast.warn("Select 2 Insignias Only");
                    return prevItems;
                } else {
                    scrafArray.push(item);
                }
            }
    
            scrafArray?.sort((a, b) => a.id - b.id);
    ...
    }, [currentDresses?.keyName,selectedOptions?.leftBiceps,]);

    useEffect(() => {
        let scrafCords = GetScrafCord(currentDresses?.keyName);

        if (scrafCords) {
            setPositions((prev) => {
                return {
                    ...prev,
                    scrafCords: scrafCords,
                };
            });
        }
        ...
        >
            <Box
                ...
                sx={{
                    ...
                    //padding: "0.5rem 0",
                    backgroundColor: "rgba(11, 12, 73, 0.3)", /* 0.7 makes it semi-transparent */
                    borderTop: "2px solid rgba(255, 255, 255, 0.3)", /* soft white transparent line */
                    padding: "0.5rem 0",
                }}
            >
                        ...
                        <List className="flex_center" sx={{ gap: "1rem", flexWrap: "wrap" }}>
                            ...
                            {editHeaderMenu.map((item, index) => (
                                <CustomListItem
                                    ...
                                    sx={{
                                        ...
                                        whiteSpace: "nowrap",
                                        background:
                                            currentTab === item.tabNum ? "white" : "#0B0B45",
                                        color: currentTab === item.tabNum ? "#0B0B45" : "white",
                                    }}
                                >
                                    {item.tabNam}
                                </CustomListItem>
                            ...
                            <CustomListItem
                                ...
                                sx={{
                                    ...
                                    width: "fit-content",
                                    whiteSpace: "nowrap",
                                    background: currentTab === 10 ? "white" : "#0B0B45",
                                    color: currentTab === 10 ? "#0B0B45" : "white",
                                }}
                            >
                                Specifications
                            </CustomListItem>
                    ...
                    >
                        ...
                        >
                            <Box sx={{ width: "20rem", height: "50rem", position: "relative" }}>
                                ...
                                currentDresses?.keyName === "CeremonialSummerForCPOSANDPOS" ||
                                currentDresses?.keyName === "WorkingDressWhiteSummerCPOsPOs" ||
                                currentDresses?.keyName === "CeremonialSummerForPOS" ||
                                currentDresses?.keyName === "WorkingDressWhiteSummerPOs" ||
                                currentDresses?.keyName === "CeremonialSummerForLDGSAndBelow" ||
                                ...
                                >
                                    ...
                                    >
                                        <img
                                            ...
                                            alt="unifrom-logos"
                                            title={
                                                currentDresses?.keyName === "femaleServiceDressBlackAFNS" || currentDresses?.keyName === "femaleFullBlackWinterCeremonialDressAFNS" || currentDresses?.keyName === "femaleFullBlackWinterCeremonialDress" || currentDresses?.keyName === "femaleServiceBlackWinterCeremonialDress" || currentDresses?.keyName === "FullBlackWinterCeremonialDress" || currentDresses?.keyName === "ServiceBlackDressWinter" 
                                                ? "Above name tally, center-aligned"
                                                : selectedOptions?.leftPocketInsignia?.length !== 3 || selectedOptions?.rightChest?.length > 0 ? "Pak flag mast is almost touching the upper line of name tally" : "Pak Flag is almost touching the upper edge of insignia"
                                    ...
                                    {!currentDresses?.tally && (
                                        <Box
                                            className="flex_center"
                                            title={currentDresses?.keyName === "femaleServiceDressBlackAFNS" || currentDresses?.keyName === "femaleFullBlackWinterCeremonialDressAFNS" || currentDresses?.keyName === "femaleFullBlackWinterCeremonialDress" || currentDresses?.keyName === "femaleServiceBlackWinterCeremonialDress" || currentDresses?.keyName === "FullBlackWinterCeremonialDress" || currentDresses?.keyName === "ServiceBlackDressWinter"
                                                ? currentDresses?.velcroText ? "Name Tally Should be With Velcro and right above the right pocket" : currentDresses?.keyName === "MessDressWhiteSummerMessKit"|| currentDresses?.keyName === "MessDressBlackWinterMessKit" || currentDresses?.keyName === "femaleMessDressSummerMessKit"|| currentDresses?.keyName === "femaleMessDressBlackWinterMessKit" || currentDresses?.keyName === "femaleMessDressSummerMessKitAFNS"|| currentDresses?.keyName === "femaleMessDressBlackWinterMessKitAFNS"  ? "to be worn on a bar on the right lapel extending toward the shoulder and should use the small one." : "to be worn on a bar on the right lapel extending toward the shoulder" 
                                ...
                                {selectedOptions?.rank?.map((item) => {
                                    ...
                                    let hoverData=currentDresses?.hoverData;
                                    let findHoverData = hoverData?.find(
                                        (data) => data?.holderName === "Ranks"
                                    );
                                    const title_r = item?.left_arm
                                    ? (findHoverData?.title_left ? `${findHoverData?.title_left}` : item?.name)
                                    : item?.right_arm
                                    ? (findHoverData?.title_right ? `${findHoverData?.title_right}` : item?.name)
                                    : item?.gbc
                                ...
                                <Box
                                    sx={{
                                        ...
                                        justifyContent: "center",
                                        position: "absolute",
                                        top: positions?.scrafCords?.cord_one,
                                        left: positions?.scrafCords?.cord_two,
                                        width: "1.5rem",
                                    }}
                                >
                                        ...
                                        currentItems?.map((item, index) => {
                                            return (
                                                <BadgeModel
                                                    item={item}
                                                    key={item?.id}
                                                    ToggleOptions={ToggleLeftBicep} // ✅ Directly call specific toggle function
                                                    selectedOptions={selectedOptions}
                                                />
                                            );
                                        })}
